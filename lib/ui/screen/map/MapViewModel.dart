      ...
      print('unable to retrieve');
        ...
        for (int i = 0; i < rcenterList.length; i++) {
          markers.add(new Marker(
            ...
            position: LatLng(rcenterList[i]['lat'].toDouble(), rcenterList[i]['lon'].toDouble()),
          ));
        }
  ...
  var lat, lon;
    ...
    if(lat!=null&&lon!=null){
      for(int i=0; i<nearbyCenterList.length; i++){
        if(calculateDistance(lat,lon,nearbyCenterList[i]['lat'].toDouble(),nearbyCenterList[i]['lat'].toDouble())>5){
